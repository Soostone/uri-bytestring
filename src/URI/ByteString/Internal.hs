...
-------------------------------------------------------------------------------
-- | Strict URI Parser config that doesn't parse a query string.
strictURIParserOptions' :: URIParserOptions
strictURIParserOptions' =  URIParserOptions {
      upoValidQueryChar = validForQueryString
    , upoParseQuery = False
    }
...
-------------------------------------------------------------------------------
-- | Lax URI Parser config that doesn't parse a query string.
--
laxURIParserOptions' :: URIParserOptions
laxURIParserOptions' = URIParserOptions {
      upoValidQueryChar = validForQueryStringLax
    , upoParseQuery = False
    }
...
-- todo: needs scheme, authority and path
...
serializeQuery URINormalizationOptions {..} (Query ps) =
  ...
  where
    ...
    ps'
      ...
      | otherwise = ps
serializeQuery _ NoQueryString = mempty
serializeQuery _ (QueryString s) = BB.fromByteString s
...
--
-- >>> let myLaxOptions = URIParserOptions { upoValidQueryChar = liftA2 (||) (upoValidQueryChar strictURIParserOptions) (inClass "[]") , upoParseQuery = True }
...
queryParser opts = do
  ...
  case mc of
    Just c
      | c == question -> skip' 1 *> (if upoParseQuery opts then itemsParser else queryReader)
      | c == hash     -> if upoParseQuery opts then pure mempty else return NoQueryString
      | otherwise     -> fail' MalformedPath
    _      -> if upoParseQuery opts then pure mempty else return NoQueryString
  where
    itemsParser = Query . filter neQuery <$> A.sepBy' (queryItemParser opts) (word8' ampersand)
    queryReader = QueryString <$> (A.takeWhile (upoValidQueryChar opts) `orFailWith` MalformedQuery)
...
-------------------------------------------------------------------------------
-- | Parse a query string generated by an HTML form.
parseQueryString :: URIParserOptions -> ByteString -> Either URIParseError [(ByteString,ByteString)]
parseQueryString opts = parseOnly' OtherError p
  where
    p = return . filter neQuery =<< A.sepBy' (queryItemParser opts) (word8' ampersand)
    neQuery (k, _) = not (BS.null k)
...
-------------------------------------------------------------------------------
validForQueryString :: Word8 -> Bool
validForQueryString = inClass ('?':'/':pchar)
...
-------------------------------------------------------------------------------
validForQueryStringLax :: Word8 -> Bool
validForQueryStringLax = notInClass "#"
